...
def _render_autonomous_lab_view(ctx: dict[str, object]) -> None:
    ...
    for name in ("Stocks", "Crypto", "Forex", "Metals", "International", "Kalshi Predictions", "Kalshi Perps"):
        ...
        rows.append({"Engine": name, "Status": status, "Activity Health": provider_health if is_kalshi else values.get("activity_health", "UNKNOWN"), "Last Cycle": provider.get("observed_at", values.get("latest", "UNKNOWN")), "Markets Scanned": provider.get("markets", provider.get("instruments", values.get("observations", "UNKNOWN"))), "Strategy Evaluations": provider_funnel.get("strategy_evaluations", values.get("strategy_evaluations", daily_values.get("observations", "UNKNOWN"))) if is_kalshi else values.get("strategy_evaluations", daily_values.get("observations", "UNKNOWN")), "Candidates": provider_funnel.get("scanned", values.get("candidates", daily_values.get("candidates", "UNKNOWN"))) if is_kalshi else values.get("candidates", daily_values.get("candidates", "UNKNOWN")), "Signals": values.get("signals", daily_values.get("signals", "UNKNOWN")), "Positive Edge/Proxy": provider_funnel.get("positive_edge", values.get("positive_edge_or_proxy", "UNKNOWN")) if is_kalshi else values.get("positive_edge_or_proxy", "UNKNOWN"), "Qualified": provider_funnel.get("qualified", values.get("qualified", daily_values.get("qualified", "UNKNOWN"))) if is_kalshi else values.get("qualified", daily_values.get("qualified", "UNKNOWN")), "Actual Orders": provider.get("orders", values.get("actual_orders", "UNKNOWN")) if is_kalshi else values.get("actual_orders", "UNKNOWN"), "Shadow Trades": shadow_values.get("entries", "UNKNOWN"), "Fills": provider.get("fills", values.get("fills", "UNKNOWN")) if is_kalshi else values.get("fills", "UNKNOWN"), "Open Actual": open_actual, "Open Shadow": (shadow_values.get("entries", 0) - shadow_values.get("completed", 0)) if shadow_values else "UNKNOWN", "Actual Exits": actual_values.get("completed_trades", values.get("actual_exits", "UNKNOWN")), "Shadow Exits": shadow_values.get("completed", "UNKNOWN"), "Actual Expectancy": actual_expectancy, "Shadow Expectancy": shadow_values.get("hypothetical_expectancy", "UNKNOWN"), "Realized P&L": realized_pnl, "Unrealized P&L": provider.get("unrealized_pnl", actual_provider.get("unrealized_pnl", "UNKNOWN")) if is_kalshi else actual_provider.get("unrealized_pnl", "UNKNOWN"), "Learning Observations": values.get("learning_observations", daily_values.get("observations", "UNKNOWN")), "Top Bottleneck": top_bottleneck, "Regime": (next(iter((daily_values.get("regimes") or {}).keys()), "UNKNOWN"))})
    # The campaign checkpoint measures unique markets, while the row builder
    # historically used observation count as a fallback. Normalize only when
    # the authoritative value exists; preserve UNKNOWN for unavailable data.
    for row in rows:
        engine_values = engines.get(row["Engine"])
        if row["Engine"] not in {"Kalshi Predictions", "Kalshi Perps"} and isinstance(engine_values, dict):
            if "markets_scanned" in engine_values:
                row["Markets Scanned"] = engine_values["markets_scanned"]
    st.dataframe(rows, use_container_width=True, hide_index=True)
    st.markdown("### Provider health")
    provider_rows = []
    provider_jobs = {

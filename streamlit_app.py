...
def _render_autonomous_lab_view(ctx: dict[str, object]) -> None:
    ...
    for name in ("Stocks", "Crypto", "Forex", "Metals", "International", "Kalshi Predictions", "Kalshi Perps"):
        ...
        rows.append({"Engine": name, "Activity Health": provider_health if is_kalshi else values.get("activity_health", "UNKNOWN"), "Last Cycle": provider.get("observed_at", values.get("latest", "UNKNOWN")), "Markets Scanned": provider.get("markets", provider.get("instruments", values.get("observations", "UNKNOWN"))), "Strategy Evaluations": daily_values.get("observations", "UNKNOWN"), "Candidates": provider_funnel.get("scanned", daily_values.get("candidates", "UNKNOWN")) if is_kalshi else daily_values.get("candidates", "UNKNOWN"), "Signals": values.get("signals", daily_values.get("signals", "UNKNOWN")), "Positive Edge/Proxy": provider_funnel.get("positive_edge", "UNKNOWN") if is_kalshi else "UNKNOWN", "Qualified": provider_funnel.get("qualified", values.get("qualified", daily_values.get("qualified", "UNKNOWN"))) if is_kalshi else values.get("qualified", daily_values.get("qualified", "UNKNOWN")), "Actual Orders": provider.get("orders", "UNKNOWN") if is_kalshi else "UNKNOWN", "Shadow Trades": "UNKNOWN", "Fills": provider.get("fills", "UNKNOWN") if is_kalshi else "UNKNOWN", "Open Actual": "UNKNOWN", "Open Shadow": "UNKNOWN", "Actual Exits": "UNKNOWN", "Shadow Exits": "UNKNOWN", "Actual Expectancy": "UNKNOWN", "Shadow Expectancy": "UNKNOWN", "Realized P&L": "UNKNOWN", "Unrealized P&L": provider.get("unrealized_pnl", "UNKNOWN") if is_kalshi else "UNKNOWN", "Learning Observations": daily_values.get("observations", "UNKNOWN"), "Top Bottleneck": (provider.get("last_rejection_reason", daily_values.get("top_bottlenecks")) if is_kalshi else (daily_values.get("top_bottlenecks") or "UNKNOWN")), "Regime": (next(iter((daily_values.get("regimes") or {}).keys()), "UNKNOWN"))})
    st.dataframe(rows, use_container_width=True, hide_index=True)
    st.markdown("### Provider health")
    provider_rows = []
    provider_jobs = {

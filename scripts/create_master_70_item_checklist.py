...
def _status(item_id: str, safety: dict[str, object], runtime: dict[str, object], progress: dict[str, object], validation: dict[str, object] | None = None) -> tuple[str, str]:
    ...
    if item_id in {"A", "B"}:
        ...
        clean_window = runtime_evidence.get("unresolved_runtime_failures") == 0 and runtime_evidence.get("consecutive_successful_autonomous_cycles", 0) >= 3
        if progress.get("runtime", {}).get("healthy") and clean_window:
            return ("PASS", "current health and three-or-more consecutive successful cycles")
        return ("UNKNOWN", "runtime evidence is incomplete")
    if item_id == "AA":
    ...
    source_markers = {
        ...
        "P": ("scripts/create_activity_lab_report.py", "def _bottleneck"),
        "R": ("src/autotrader/daily_report.py", "p95_latency_ms_job_proxy"),
        "S": ("src/autotrader/dashboard_app.py", "AUTONOMOUS TRADING LAB"),
        ...
        **{str(number): ("src/autotrader/multi_strategy.py", marker) for number, marker in {
        ...
        "29": ("scripts/kalshi_execution_cycle.py", '"side": "bid"'),
        ...
        "38": ("src/autotrader/risk.py", "portfolio.positions.get(proposal.symbol)"),

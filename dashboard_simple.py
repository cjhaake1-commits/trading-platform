...
def current_view():
    ...
    if fresh(report.get("positions_observed_at"), now):
        st.caption("These are provider observations, including older or unattributed holdings. They are not automatically credited to the current test. Non-USD values are not summed into USD totals.")
    ...
    if recent and first.get("qualified") is True:
        ...
        st.write("No current verified post-boundary order/fill is available in this report.")
    ...
    st.caption("Short sales, options and leveraged strategies must have verified provider support and pass existing risk controls. No live-money trading is enabled by this app.")

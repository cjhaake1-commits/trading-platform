...
def test_four_pillar_capital_is_4000():
    assert TOTAL_PAPER_CAPITAL == 4000.0
    assert set(PILLAR_ALLOCATIONS) == {"alpaca_equities", "oanda_fx", "alpaca_crypto", "ibkr_global"}
...
def test_corrupt_or_forbidden_learning_cannot_mutate_guardrails(tmp_path):
    ...
    path.write_text(json.dumps({"risk_per_trade_pct": 0.99, "minimum_candidate_score": 999}), encoding="utf-8")
...
def test_learning_waits_for_minimum_samples(tmp_path):
    ...
    with sqlite3.connect(db) as con:
        con.execute("CREATE TABLE fills (broker TEXT, symbol TEXT, side TEXT, quantity REAL, price REAL, realized_pnl REAL, occurred_at TEXT, metadata_json TEXT)")
        for i in range(19):
            con.execute("INSERT INTO fills VALUES ('paper','SPY','sell',1,100,1,?, '{}')", (f"2026-08-17T00:{i:02d}:00+00:00",))
    learner = RealizedOutcomeLearner(ledger_path=str(db), stats_path=str(tmp_path/'stats.json'), parameters_path=str(tmp_path/'params.json'), history_path=str(tmp_path/'history.jsonl'))

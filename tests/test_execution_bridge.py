...
class Adapter:
    ...
    def submit(self, intent):
        self.calls += 1
        return {"provider_order_id": "PAPER-1", "ack": True}


def test_bridge_fails_closed_before_adapter(tmp_path):
    adapter = Adapter()
    result = execute_qualified_queue_record(
        {"execution_mode": "PAPER", "qualified_signal": True},
        adapter=adapter,
...
def test_bridge_requires_owned_position_and_records_intent(tmp_path):
    adapter = Adapter()
    ledger = ForwardLifecycle(tmp_path / "l.db")
    record = {
        "execution_mode": "PAPER",
        "qualified_signal": True,
        "risk_approved": True,

...
def test_final_report_gate_is_not_hardcoded_when_checklist_is_present(tmp_path, monkeypatch):
    ...
    items = [{"id": index, "status": "PASS"} for index in range(1, 71)]
    (report_dir / "master-70-item-checklist.json").write_text(__import__("json").dumps({"items": items}))
...
def test_final_report_does_not_verify_empty_checklist(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    report_dir = tmp_path / "var/reports"
    report_dir.mkdir(parents=True)
    ...
    (report_dir / "overnight-progress.json").write_text('{"runtime":{"healthy":true},"git_sha":"abc"}')
    assert build_report().startswith("HIGH_ACTIVITY_PAPER_LAB_V1 — NOT YET VERIFIED")


def test_final_report_does_not_verify_partial_checklist(tmp_path, monkeypatch):
    monkeypatch.chdir(tmp_path)
    report_dir = tmp_path / "var/reports"
    report_dir.mkdir(parents=True)
    (report_dir / "master-70-item-checklist.json").write_text('{"items":[{"id":1,"status":"PASS"}]}')
    (report_dir / "overnight-forward-campaign.json").write_text(
        '{"safety":{"live_trading_enabled":false,"real_money_orders":0},"runtime_evidence":{"unresolved_runtime_failures":0},"engines":{},"providers":{}}'
    )
    (report_dir / "overnight-progress.json").write_text('{"runtime":{"healthy":true},"git_sha":"abc"}')

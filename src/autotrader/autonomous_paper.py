...
from .strategy_health import (
    load_crypto_runtime_health,
    load_persisted_health,
    rank_crypto_opportunities,
    rank_opportunities,
)
        ...
        for instrument, bars in histories.items():
            ...
            if candidate is not None:
                ...
                if instrument.asset_class is AssetClass.CRYPTO:
                    ...
                    health = {**load_persisted_health(), **load_crypto_runtime_health()}
                    ranked_learning = rank_crypto_opportunities(
                    ...
                    if ranked_learning and not any(item["execution_eligible"] for item in ranked_learning):
                        # Preserve the rejected opportunity as a durable,
                        # provider-free counterfactual.  Governance must be
                        # measurable without creating a broker order.
                        if experiment_id and candidate is not None:
                            self.experiment_ledger.record_counterfactual(
                                symbol=instrument.symbol, occurred_at=bars[-1].timestamp,
                                champion_decision="REJECT_GOVERNANCE",
                                challenger_decision="REJECT_GOVERNANCE",
                                entry_price=candidate.last_price, quantity=1000.0 / candidate.last_price if candidate.last_price > 0 else 0.0,
                                stop_price=candidate.suggested_stop, target_price=None,
                                candidate_identity=f"{bars[-1].timestamp.isoformat()}|{candidate.score:.6f}|governance",
                                features={"governance": ranked_learning, "tag": "CRYPTO_NEGATIVE_COHORT_BLOCK"},
                            )

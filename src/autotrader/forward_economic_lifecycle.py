...
class EconomicLifecycle:
    def __init__(self, path: str | Path = "var/autotrader/economic-lifecycle.db") -> None:
        ...
        with sqlite3.connect(self.path) as db:
            db.execute(
                "CREATE TABLE IF NOT EXISTS events (event_id TEXT PRIMARY KEY, trade_id TEXT, stage TEXT, ownership TEXT, payload TEXT)"
            )
            db.execute(
                "CREATE TABLE IF NOT EXISTS releases (trade_id TEXT PRIMARY KEY, amount REAL NOT NULL, realized_pnl REAL, released_at TEXT NOT NULL)"
            )
    ...
    def release(self, trade_id: str, *, amount: float, realized_pnl: float | None, released_at: str) -> bool:
        if amount < 0:
            raise ValueError("released capital cannot be negative")
        with sqlite3.connect(self.path) as db:
            if db.execute("SELECT 1 FROM releases WHERE trade_id=?", (trade_id,)).fetchone():
                return False
            if not db.execute(
                "SELECT 1 FROM events WHERE event_id=? AND stage='EXIT_FILL' AND ownership='PLATFORM_OWNED'",
                (f"{trade_id}:EXIT_FILL",),
            ).fetchone():
                return False
    ...
    def metrics(self) -> dict[str, float]:
        with sqlite3.connect(self.path) as db:
            released, pnl = db.execute(
                "SELECT COALESCE(SUM(amount),0), COALESCE(SUM(realized_pnl),0) FROM releases"
            ).fetchone()
            outcomes = db.execute(
                "SELECT COUNT(*) FROM events WHERE stage='OUTCOME' AND ownership='PLATFORM_OWNED'"
            ).fetchone()[0]
            redeployed = db.execute(
                "SELECT COALESCE(SUM(CAST(json_extract(payload, '$.capital_required') AS REAL)),0) FROM events WHERE stage='ORDER_INTENT' AND ownership='PLATFORM_OWNED'"
            ).fetchone()[0]
        return {
            "capital_released": float(released),
            "realized_pnl": float(pnl),
            "completed_outcomes": int(outcomes),
            "capital_redeployed": float(redeployed),
    ...
    def velocity_metrics(self) -> dict[str, float | None]:
        ...
        with sqlite3.connect(self.path) as db:
            ...
            intents = db.execute(
                "SELECT trade_id, payload FROM events WHERE stage='ORDER_INTENT' AND ownership='PLATFORM_OWNED' ORDER BY event_id"
            ).fetchall()
        ...
        for _, _, released_at in releases:
            ...
            for _, payload in intents:
                try:
                    allocated_at = datetime.fromisoformat(
                        str(json.loads(payload).get("occurred_at")).replace("Z", "+00:00")
                    )
        ...
        return {
            "capital_released": released_total,
            "capital_redeployed": redeployed,
            ...
            "capital_velocity": len(latencies)
            / max(
                (
                    datetime.now().astimezone() - datetime.fromisoformat(str(releases[0][2]).replace("Z", "+00:00"))
                ).total_seconds()
                / 86400,
                1.0,
            )

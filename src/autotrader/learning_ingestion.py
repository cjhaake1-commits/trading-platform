...
class LearningIngestor:
    ...
    def ingest(self, *, outcome_id: str, outcome: Mapping[str, object]) -> bool:
        ...
        if ownership != "PLATFORM_OWNED":
            return False
        decision_at = outcome.get("decision_at")
        outcome_at = outcome.get("outcome_at")
        if (
            not decision_at
            or not outcome_at
            or datetime.fromisoformat(str(outcome_at).replace("Z", "+00:00"))
        ...
        with sqlite3.connect(self.path) as db:
            cur = db.execute(
                ...
                (
                    outcome_id,
                    str(outcome.get("strategy") or "UNKNOWN"),
                    str(outcome.get("pillar") or "UNKNOWN"),
                    str(outcome.get("instrument") or "UNKNOWN"),
                    str(outcome.get("regime") or "UNKNOWN"),
                    str(outcome.get("direction") or "UNKNOWN"),
                    ownership,
                    str(decision_at),
                    str(outcome_at),
                    outcome.get("realized_pnl"),
                    json.dumps(dict(outcome), sort_keys=True, default=str),
                ),
            )
    ...
    def count(self) -> int:
        with sqlite3.connect(self.path) as db:
            return int(db.execute("SELECT COUNT(*) FROM outcomes").fetchone()[0])
    ...
    def strategy_health(self, *, minimum_sample: int = 30) -> list[dict[str, object]]:
        ...
        with sqlite3.connect(self.path) as db:
            rows = db.execute(
                "SELECT strategy, COUNT(*), AVG(realized_pnl) FROM outcomes WHERE ownership='PLATFORM_OWNED' GROUP BY strategy"
            ).fetchall()
        return [
            {
                **assess_strategy_health(
                    ...
                    "v1",
                    int(sample),
                    float(expectancy) if expectancy is not None else None,
                    minimum_sample=minimum_sample,
                ),
                "source": "CLEAN_FORWARD_PAPER_OUTCOMES",
            }
            for strategy, sample, expectancy in rows
        ]

    def publish_strategy_health(
        self, path: str | Path = "var/reports/strategy-health.json", *, minimum_sample: int = 30
    ) -> dict[str, object]:
        payload = {
            "strategies": self.strategy_health(minimum_sample=minimum_sample),
            "evidence_scope": "FORWARD_PAPER_PLATFORM_OWNED_ONLY",
            "minimum_sample": minimum_sample,
        }
        target = Path(path)

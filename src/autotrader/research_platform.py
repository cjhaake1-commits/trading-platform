...
class ResearchStore:
    ...
    def put_research(self, record: Mapping[str, Any]) -> None:
        ...
        values["metadata_json"] = (
            _json(values.get("metadata_json", {}))
            if not isinstance(values.get("metadata_json"), str)
            else values["metadata_json"]
        )
        values["broker_control"] = 0
        with self.connection() as conn:
            conn.execute(
                f"INSERT OR REPLACE INTO research_records ({','.join(fields)}) VALUES ({','.join('?' for _ in fields)})",
    ...
    def research(self, lane: str | None = None) -> list[dict[str, Any]]:
        with self.connection() as conn:
            rows = conn.execute(
                "SELECT * FROM research_records" + (" WHERE lane=?" if lane else "") + " ORDER BY retrieved_at DESC",
                ([lane] if lane else []),
            ).fetchall()
    ...
    def put_feature(
        ...
        *,
        ...
        with self.connection() as conn:
            conn.execute(
                "INSERT INTO research_features(feature_name,feature_value,source,recorded_at,freshness,experiment_id,symbol,pillar) VALUES(?,?,?,?,?,?,?,?)",
                (name, value, source, utc_now(), freshness, experiment_id, symbol, pillar),
            )

    def put_regime(self, regime: str, evidence: Mapping[str, Any], experiment_id: str) -> None:
        with self.connection() as conn:
            conn.execute(
                "INSERT INTO regimes(regime,evidence_json,recorded_at,experiment_id) VALUES(?,?,?,?)",
                (regime, _json(evidence), utc_now(), experiment_id),
            )
    ...
    def put_hedge(self, observation: Mapping[str, Any]) -> None:
        ...
        with self.connection() as conn:
            conn.execute(
                f"INSERT INTO hedge_observations ({','.join(fields)},recorded_at) VALUES ({','.join('?' for _ in fields)},?)",
                [observation.get(f) for f in fields] + [utc_now()],
            )
    ...
    def put_cash(self, experiment_id: str, **values: float) -> None:
        names = (
            ...
            "fees_costs",
            "net_realized_cash",
            "liquid_realized_cash",
            "harvested_cash",
            "redeployable_cash",
            "unrealized_pnl",
            "theoretical_compounded_equity",
        )
        ...
        with self.connection() as conn:
            conn.execute(
                "INSERT OR REPLACE INTO cash_buckets VALUES (?,?,?,?,?,?,?,?,?,?,?,?)",
                (experiment_id, *[payload[n] for n in names], utc_now()),
            )
    ...
    def put_provider_status(
        ...
        *,
        ...
        with self.connection() as conn:
            conn.execute(
                "INSERT OR REPLACE INTO provider_status(lane,status,last_success,last_attempt,next_refresh,records_ingested,last_error) VALUES(?,?,?,?,?,?,?)",
                (lane, status, now if status == "CONNECTED" else None, now, next_refresh, records_ingested, last_error),
            )
    ...
    def put_attribution(
        ...
        *,
        ...
        with self.connection() as conn:
            conn.execute(
                "INSERT INTO feature_attribution(observation_id,feature_name,feature_value,feature_source,feature_freshness,feature_weight,regime,model,decision,outcome,realized_contribution,confidence,recorded_at) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?)",
                (
                    observation_id,
                    feature_name,
                    feature_value,
...
def performance_metrics(closes: list[float], *, periods_per_year: int = 252) -> dict[str, float | None]:
    ...
    return {
        ...
        "risk_adjusted_return": (mean_return / volatility * sqrt(periods_per_year)) if volatility else None,
        "trend_persistence": persistence,
    }


def normalize_disclosure(
    *, lane: str, source: str, source_url: str, as_of_date: str, payload: Mapping[str, Any]
) -> dict[str, Any]:
    ...
    return {
        ...
        "freshness": "DELAYED",
        "instrument": payload.get("symbol") or payload.get("asset"),
        "signal_type": payload.get("signal_type", "structural"),
        "signal_value": payload.get("signal_value"),
        "confidence": float(payload.get("confidence", 0.0) or 0.0),
        "metadata_json": dict(payload),
        "backtest_status": payload.get("backtest_status", "NOT_STARTED"),
        "walk_forward_status": payload.get("walk_forward_status", "NOT_STARTED"),
        "paper_shadow_status": payload.get("paper_shadow_status", "NOT_STARTED"),
        "promotion_status": payload.get("promotion_status", "RESEARCH"),
        "model_weight": 0.0,
        "broker_control": 0,
    }
...
def classify_regime(
    *, return_pct: float, volatility: float, trend_strength: float, gap_pct: float = 0.0, liquidity_score: float = 1.0
) -> str:
...
def evaluate_shadow_hedge(
    *,
    overnight_direction: float,
    gap_pct: float,
    volatility: float,
    ...
    return {
        "candidate": candidate,
        "instrument": "INDEX_FUTURE_RESEARCH",
        "mode": mode if mode in {"EXECUTABLE_SIM", "SHADOW_ONLY"} else "SHADOW_ONLY",
        "reason": "opening risk concentration" if candidate else "opening risk below threshold",
        "score": score,
        "recommended_size": round(score * 0.10, 6),
        "risk_before": portfolio_concentration,
        "risk_after": max(0.0, portfolio_concentration - score * 0.10),
...
def apply_v2_exit(
    ...
    *,
    ...
    store.put_cash(
        ...
        fees_costs=fees,
        net_realized_cash=net,
        liquid_realized_cash=max(net, 0.0),
        redeployable_cash=max(net, 0.0),
...
def build_daily_report(
    *,
    ...
    realized_cash: float,
    liquid_cash: float,
    redeployable_cash: float,
    harvested_cash: float,
    ...
    pillar_attribution: Mapping[str, float] | None = None,
    **extra: Any,
) -> dict[str, Any]:
    return {
        "date": report_date,
        ...
        "redeployable_cash": redeployable_cash,
        "harvested_cash": harvested_cash,
        "unrealized_pnl": unrealized_pnl,
        "daily_return": (ending_equity / starting_equity - 1) if starting_equity else 0.0,
        "trades": trades,
        ...
        "pillar_attribution": dict(pillar_attribution or {}),
        **extra,
    }

...
def _sync_submitted_position(
    *,
    ...
    """Sync broker truth after a successfully submitted paper/practice entry.

    If expected_quantity is supplied, keep polling until the broker position has
    reached that quantity. This avoids persisting a transient partial fill.
    """
    ...
    if expected_quantity is not None and abs(broker_position.quantity) + 1e-12 < abs(expected_quantity):
        raise RuntimeError(
            f"Order was submitted for {expected_quantity} {normalized_symbol}, but broker position "
            f"only reached {broker_position.quantity} during sync; ledger was not changed."
        )

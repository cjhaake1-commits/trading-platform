...
from .execution_bridge import PaperAdapter, execute_qualified_queue_record
from .forward_economic_lifecycle import EconomicLifecycle
from .forward_lifecycle import ForwardLifecycle

REJECTIONS = {
    "INSUFFICIENT_CANONICAL_ECONOMICS",
    "STALE_EVIDENCE",
...
class RuntimeExecutionConsumer:
    def __init__(
        self,
        path: str | Path = "var/autotrader/execution-intents.db",
        lifecycle: ForwardLifecycle | None = None,
        economic: EconomicLifecycle | None = None,
    ) -> None:
        self.path = Path(path)
    ...
    def consume(
        self, record: Mapping[str, object], *, adapter: PaperAdapter | None = None, now: str = ""
    ) -> dict[str, object]:
        ...
        with sqlite3.connect(self.path) as db:
            existing = db.execute(
                "SELECT state, reason FROM execution_intents WHERE intent_id=?", (intent_id,)
            ).fetchone()
        if existing:
        ...
        return self._persist(
            intent_id, state, None if state == "SUBMITTED" else str(result.get("reason")), record, result
        )
        ...
        with sqlite3.connect(self.path) as db:
            db.execute(
                ...
                (
                    ...
                    state,
                    reason,
                    json.dumps({**dict(record), "result": dict(result or {})}, sort_keys=True, default=str),
                ),
            )
        return {
            "state": state,
            "reason": reason,
            "side_effects": "NONE" if state == "REJECTED" else "PAPER_PROVIDER_ONLY",
        }
